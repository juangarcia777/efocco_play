...
// VERIFICA SE O CPF ALTERADO JÁ NÃO EXISTE
	...
	if ($db->rows($grava)) {
		...
		$grava = $db->select("UPDATE users SET data_cadastro='$data_cadastro', senha='$senha', situacao='$situacao', nome='$nome_aluno', email_aluno='$email_aluno', celular_aluno='$celular_aluno', telefone_aluno='$telefone_aluno', telefone_trab_aluno='$telefone_trab_aluno', cpf='$cpf_aluno', usuario='$cpf_aluno',  rg='$rg_aluno', cep='$cep', municipio_nascimento='$municipio_nascimento', logradouro='$logradouro', bairro='$bairro', municipio='$municipio', estado='$estado' WHERE id='$id_aluno' LIMIT 1");
