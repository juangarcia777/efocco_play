...
$verifica = $db->select("SELECT * FROM users WHERE cpf = '$cpf_aluno' OR nome = '$nome_aluno'");
		...
		if ($db->rows($verifica)) {
			...
			$_SESSION['aviso-mensagem-ava'] = 'Ops! Esse CPF já está cadastrado.';
 			...
 			$grava = $db->select("INSERT INTO users (usuario, senha, tipo, nome, email_aluno, telefone_aluno, celular_aluno, telefone_trab_aluno, cpf, rg, municipio_nascimento, logradouro, bairro, municipio, estado, cep, situacao, data_cadastro) VALUES ('$usuario', '$senha', '$tipo', '$nome_aluno', '$email_aluno', '$telefone_aluno', '$celular_aluno', '$telefone_trab_aluno', '$cpf_aluno', '$rg_aluno', '$municipio_nascimento', '$logradouro', '$bairro', '$municipio', '$estado', '$cep', 'Ativo', '$data_cadastro')");	
